...
const Ideas = props => {
  ...
  useEffect(() => {
    const unsubscribe = ideasCollection.orderBy("timestamp", "desc").onSnapshot(({docs}) => {
      const ideasFromDB = [];
  ...
  const renderIdeas = () => {
    ...
    return ideas.map(idea => (<Idea key={idea.id} idea={idea} onDelete={onIdeaDelete}/>));
  };
  ...
  return (<div>
    ...
    <form className="app__footer" onSubmit={onIdeaAdd}>
      <input type="text" className="app__footer__input" placeholder="Add a new idea" value={idea} onChange={e => setIdeaInput(e.target.value)}/>
